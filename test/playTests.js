...
module.exports = {
	...
	loadingPartialDoesntAffectGame : function(test){
		...
		console.log('partial', partial.players.map(p=>p.hand.map(c=>[c.color,c.number].join(' ')).join(' - ')).join('  --  '));
		console.log('hands', game.players.map(p=>p.hand.map(c=>[c.color,c.number].join(' ')).join(' - ')).join('  --  '));
