...
module.exports = {
  ...
  playersKeepNames: function(test) {
    ...
    for (var name of names) {
      test.ok(
        game_init.players.map(p => p.name).indexOf(name) >= 0,
        "Player name ended up in game"
      );
    }
    test.strictEqual(
      names.length,
      game_init.players.length,
      "Game gets correct num of players"
    );
    test.done();
  },
  ...
  defaultGameHas50Cards: function(test) {
    ...
    test.strictEqual(
      50,
      game.deck.length +
        game.players.map(p => p.hand.length).reduce((o, l) => o + l),
      "Deck should have 50 cards"
    );
    test.done();
  },
  ...
  canPlayOnMyTurn: function(test) {
    ...
    const playRes = hanabi.play(
      game.players[game.whosTurn].id,
      game.players[game.whosTurn].hand[0].id
    );
  ...
  playingAtBeginningEndsWithFullHand: function(test) {
    ...
    console.log(
      "playerStartHand",
      game.players[startTurn].hand.map(c => c.id).join(", ")
    );
    hanabi.play(
      game.players[game.whosTurn].id,
      game.players[game.whosTurn].hand[0].id
    ...
    test.equal(
      game.players[startTurn].hand.length,
      5,
      "Player who played should end with full hand"
    );
    test.done();
  },
  ...
  playingAtBeginningIncrementsTurn: function(test) {
    ...
    if (game.whosTurn === 0) {
      test.equal(
        startTurn,
        names.length,
        "if new player is 0, last player should be last-player"
      );
    } else {
      test.equal(
        startTurn + 1,
        game.whosTurn,
        "Next player should be one higher than last"
      );
    }
    test.done();
  },
  ...
  playingNonOneAtBeginningDecrementBombs: function(test) {
    ...
    const game = hanabi.getGame();
    const startTurn = game.whosTurn;
    const non_ones = game.players[game.whosTurn].hand.filter(
      c => c.number !== 1
    );
